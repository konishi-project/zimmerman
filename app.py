...
with open('server_cfg.json',"r+") as conf_file:
    ...
    if os.path.isfile(database):
        ...
        db.execute("CREATE TABLE users (username,fullname,password,email,id,ip_addr,reg_date,admin,zucc,profile_pic)")
        db.execute("CREATE TABLE sessions (uid,sid,time,invalid)")
        db.execute("CREATE TABLE pictures (id,path,uid,time,og_name,nsfw)")
